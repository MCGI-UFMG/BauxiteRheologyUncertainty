#-*- coding: utf-8 -*-
#Código para extração de resultados de modelagem hidrodinâmica de arquivos .HDF
#Gabriel Fonseca Cunha e Rodrigo Perdigão Gomes Bezerra, 2024
...
#Caminho dos arquivos HDF
folder="D:/RAS_results/RAS1/"
os.chdir(folder)

#Caminho do arquivo que identifica as células de interesse (id_cells.txt)
cells_ID=pd.read_csv(r'D:/RAS_results/id_cells.txt',sep="\t")

#Caminho do arquivo que identifica as faces de interesse (id_face.txt)
...
#Caminho para arquivo excel de resultados de velocidade
with pd.ExcelWriter(r'D:/RAS_results/RAS1/vel_output.xlsx') as writer: 
    i=1
...
#Caminho para arquivo excel de resultados de WSE
with pd.ExcelWriter(r'D:/RAS_results/RAS1/wse_output.xlsx') as writer: 
    i=1
    for x in main_wse_list: 
        x.to_excel(writer, sheet_name="XS"+str(i), index=False) 
        i=i+1

#Salvando resultados de elevação mínima da célula
#Caminho para arquivo excel de resultados de elevação mínima da célula
with pd.ExcelWriter(r'D:/RAS_results/RAS1/min-El_output.xlsx') as writer: 
    i=1
...
#Inserir linhas vazias no arquivo excel, para preenchimento do nome das topografias
...
#Função para extrair a string gXXuYY (título do plan) da primeira linha de cada arquivo plan do HEC-RAS
...
#Função para atualizar os arquivos Excel com os títulos dos plans
